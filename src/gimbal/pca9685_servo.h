...
#include "board/Inc/general_stm32_config.h"
...
public:
     ...
     */
    bool moveToPreset(const std::array<uint16_t, static_cast<size_t>(ServoID::COUNT)>& positions);
     ...
     */
    [[nodiscard]] const TelemetryArray& getAllTelemetry() const { return telemetry_; }
...
private:
    ...
    uint16_t angleToPulseWidth(ServoID id, uint16_t angle_degrees);
    [[nodiscard]] bool validateCommand(const ServoCommand& command) const;
    void updateTelemetry(ServoID id, uint16_t target_angle, ServoStatus status, ServoError error = ServoError::NONE);
