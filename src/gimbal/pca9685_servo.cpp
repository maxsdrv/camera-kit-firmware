...
bool PCA9685ServoController::initialize(I2C_HandleTypeDef* hi2c) {
    ...
    data[1] = 0x00;
    if (HAL_I2C_Master_Transmit(hi2c_, PCA9685_ADDRESS << 1, data, 2, 100) != HAL_OK) {
        return false;
    }
    HAL_Delay(10);
    ...
    data[1] = 0x79; // 50Hz prescaler value
    if (HAL_I2C_Master_Transmit(hi2c_, PCA9685_ADDRESS << 1, data, 2, 100) != HAL_OK) {
        return false;
    }
...
uint16_t PCA9685ServoController::angleToPulseWidth(ServoID id, uint16_t angle_degrees) {
    const auto config = getServoConfig(id);
    
    // Clamp angle to servo constraints
    const auto& constraints = config.constraints;
    if (angle_degrees < constraints.min_angle) angle_degrees = constraints.min_angle;
    if (angle_degrees > constraints.max_angle) angle_degrees = constraints.max_angle;
    
    // Convert angle to pulse width using servo config
    uint16_t pulse_width = config.min_pulse_width_us + 
        ((angle_degrees * (config.max_pulse_width_us - config.min_pulse_width_us)) / 180);
...
bool PCA9685ServoController::validateCommand(const ServoCommand& command) const {
    ...
    switch (command.type) {
        case CommandType::MOVE_TO_POSITION:
            return (command.target_position >= constraints.min_angle && 
        ...
        case CommandType::SET_SPEED:
            return (command.speed >= constraints.min_speed &&
                   command.speed <= constraints.max_speed);
...
void PCA9685ServoController::updateTelemetry(ServoID id, uint16_t target_angle, ServoStatus status, ServoError error) {
...
void PCA9685ServoController::setPCA9685Channel(uint8_t channel, uint16_t pulse_width) const {
    if (!hi2c_ || channel >= 16) {
        return;
    }
    ...
    data[3] = pulse_width & 0xFF;        // OFF_L
    data[4] = (pulse_width >> 8) & 0xFF; // OFF_H
    
    HAL_I2C_Master_Transmit(hi2c_, PCA9685_ADDRESS << 1, data, 5, 100);
}

bool PCA9685ServoController::executeCommand(const ServoCommand& command) {
    if (!validateCommand(command)) {
        updateTelemetry(command.servo_id, command.target_position, ServoStatus::ERROR, ServoError::POSITION_LIMIT_EXCEEDED);
        return false;
    }
    
    switch (command.type) {
        case CommandType::MOVE_TO_POSITION:
            return setServoAngle(command.servo_id, command.target_position, command.speed);
        
        case CommandType::STOP:
            updateTelemetry(command.servo_id, telemetry_[servoIDToIndex(command.servo_id)].current_position, ServoStatus::IDLE);
            return true;
        
        case CommandType::HOME:
            return setServoAngle(command.servo_id, getServoConfig(command.servo_id).home_position);
...
bool PCA9685ServoController::setServoAngle(ServoID id, uint16_t angle_degrees, uint8_t speed) {
    ...
    if (angle_degrees < constraints.min_angle || angle_degrees > constraints.max_angle) {
        updateTelemetry(id, angle_degrees, ServoStatus::ERROR, ServoError::POSITION_LIMIT_EXCEEDED);
        return false;
    }
...
bool PCA9685ServoController::moveToPreset(const std::array<uint16_t, static_cast<size_t>(ServoID::COUNT)>& positions) {
    bool success = true;
...
void PCA9685ServoController::updateTelemetry() {
    ...
    for (auto& telem : telemetry_) {
        ...
        if (telem.status == ServoStatus::MOVING) {
            ...
            if (current_time - telem.timestamp > 1000) { // 1 second movement time
                telem.status = ServoStatus::IDLE;
                telem.current_speed = 0;
            }
        }
...
extern "C" {
    ...
    bool PCA9685_SetServoAngle(uint8_t servo_id, uint16_t angle_degrees) {
        if (!dof::g_servo_controller || servo_id >= static_cast<uint8_t>(dof::ServoID::COUNT)) {
            return false;
        }
        return dof::g_servo_controller->setServoAngle(dof::indexToServoID(servo_id), angle_degrees);
    }
    ...
    const char* PCA9685_GetServoStatus(uint8_t servo_id) {
        ...
        auto telem = dof::g_servo_controller->getTelemetry(dof::indexToServoID(servo_id));
        switch (telem.status) {
